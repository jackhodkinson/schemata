...
var Matrix = []Entry{
	...
	{DomainFamily, true, Pipeline{yes(), yes(), yes(), yes(), yes(), partial("owner, comment, and ACL metadata alter in place; structural changes require an explicit migration"), yes()}},
	{CompositeFamily, true, Pipeline{partial("explicit composites are captured; non-default attribute collations fail closed"), partial("attribute collations fail closed"), yes(), yes(), yes(), partial("owner, comment, and ACL metadata alter in place; structural changes require an explicit migration"), yes()}},
	{SequenceFamily, true, Pipeline{partial("permanent sequences are captured; unsupported persistence fails closed"), partial("temporary/unlogged sequences and explicit OWNED BY NONE removal fail closed"), yes(), yes(), yes(), partial("owner, comment, and ACL metadata alter in place; structural changes require an explicit migration"), yes()}},
	...
	{ViewFamily, true, Pipeline{partial("view options fail closed"), partial("output aliases, view options, and CHECK OPTION fail closed"), yes(), yes(), partial("the supported view subset includes owner, comment, and ACL metadata"), partial("metadata alters in place; definition changes use drop/create"), yes()}},
	{MaterializedViewFamily, true, Pipeline{partial("storage options, tablespaces, non-heap access methods, and unpopulated state fail closed"), partial("advanced storage/population variants fail closed"), yes(), yes(), partial("the supported materialized-view subset includes owner, comment, and ACL metadata"), partial("metadata alters in place; definition changes use drop/create"), yes()}},
	...
	{GrantOwnershipFamily, true, Pipeline{partial("object-level ACLs and owners are captured for modeled tables, views, sequences, functions, and types; column ACLs and delegated grantors fail closed"), partial("only modeled object families and exact function identities are attached; role creation is out of scope"), yes(), yes(), partial("exact metadata is emitted for supported object families; roles must already exist"), partial("exact ACL state transitions and owner changes are supported for modeled families"), partial("grants and owners are metadata rather than standalone drop targets")}},
